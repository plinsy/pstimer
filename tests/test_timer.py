...
class TestStopwatch:
    ...
    def test_multiple_start_calls(self, timer):
        ...
        assert timer._start_time == original_start_time
        assert timer.is_running()
    ...
    def test_timer_format_time(self, timer):
        ...
        # This test assumes there might be a format_time method
        # If not present, it will be skipped
        if hasattr(timer, 'format_time'):
            formatted = timer.format_time(65.432)
            assert isinstance(formatted, str)
            assert "65" in formatted  # Should contain the seconds
        else:
            pytest.skip("format_time method not implemented")
